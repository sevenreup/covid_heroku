        ...
        this.generateAll = async (req, res) => {
            ...
            if (status === 200 && data) {
                ...
                const mapped = Object.keys(countries).map(country_code => {
                    return Object.assign(Object.assign({}, countries[country_code]), { properties: Object.assign(Object.assign({}, countries[country_code].properties), { data: countryData[country_code] || { confirmed, 0: , deaths: 0, recovered: 0 } }) });
                });
                res.send(all);
            }
            console.log(status);
