...
class Cacher {
    ...
    static async getLayers() {
        const filename = path_1.default.resolve(__dirname, '../cache/layers', 'layers.json');
        let layers = null;
        try {
            layers = await fs_extra_1.default.readJson(filename);
        }
        catch (error) {
            const response = await axios_1.default.get('https://coronavirus-tracker-api.herokuapp.com/v2/latest');
