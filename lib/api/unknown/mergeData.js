...
function mergeData(dateHeaders, jhuData, dxyDate) {
    ...
    return jhuData.map((point) => {
        ...
        const base = Object.assign(Object.assign({}, point), { countryregion: listAsChina ? "China" : point.countryregion });
        ...
        if (newConfirmed)
            base[relevantDate] = newConfirmed;
        if (newRecovered)
            base["recoveries_" + relevantDate] = newRecovered;
        if (newDeath)
            base["deaths_" + relevantDate] = newDeath;
        if (!isSameDay) {
